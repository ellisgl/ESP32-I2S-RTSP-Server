...
typedef struct {
    ...
    uint16_t rtp_port;      /* client's RTP receive port       */
    bool     is_playing;
    bool     use_tcp;       /* RTP over RTSP/TCP interleaved   */
    uint16_t rtp_seq;
    uint32_t rtp_ts;
} rtsp_client_t;
...
static void build_sdp(char *out, size_t maxlen)
{
    ...
    "a=rtpmap:%d L16/%" PRIu32 "/1\r\n"
        "a=control:*\r\n",
    s_local_ip, s_local_ip, RTP_PT_L16_DYNAMIC, RTP_PT_L16_DYNAMIC, s_sample_rate);
}
                              ...
                              bool *do_teardown)
{
    *do_teardown = false;
    ...
    if (strncmp(req, "OPTIONS", 7) == 0) {
        ...
        c->rtp_port   = client_rtp_port_from(req);
        c->is_playing = false;
        c->use_tcp    = transport_requests_tcp(req);
        ...
        c->rtp_seq    = 0;
        c->rtp_ts     = 0;
        ESP_LOGI(TAG, "PLAY: client %s RTP port %d", c->peer_ip, c->rtp_port);
        return (size_t)snprintf(resp, maxlen,
            "RTSP/1.0 200 OK\r\n"
            "CSeq: %d\r\n"
            "Session: %" PRIu32 "\r\n"
            "RTP-Info: url=rtsp://%s:%d/stream;seq=%d;rtptime=0\r\n"
            "\r\n",
            cseq, c->session_id,
    ...
    while (offset < n_samples) {
        ...
        for (int i = 0; i < MAX_CLIENTS; ++i) {
            if (!s_clients[i].is_playing)
                continue;
            if (!s_clients[i].use_tcp && s_clients[i].rtp_port == 0)
                continue;

            targets[target_count].idx      = i;
            targets[target_count].sock     = s_clients[i].sock;
            targets[target_count].use_tcp  = s_clients[i].use_tcp;
            targets[target_count].rtp_port = s_clients[i].rtp_port;
            strlcpy(targets[target_count].peer_ip,
                    s_clients[i].peer_ip,
                    sizeof(targets[target_count].peer_ip));
            targets[target_count].rtp_seq = s_clients[i].rtp_seq;
            targets[target_count].rtp_ts  = s_clients[i].rtp_ts;
            ++target_count;

            ++s_clients[i].rtp_seq;
            s_clients[i].rtp_ts += (uint32_t)chunk;
        }
